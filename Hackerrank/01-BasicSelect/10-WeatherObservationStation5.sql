...
select s1.CITY, s1.namelength from (select * from (select CITY, length(CITY) namelength, ROW_NUMBER() OVER (PARTITION BY length(CITY) order by CITY) as RN from STATION ) where RN=1)s1 , (SELECT max(length(CITY)) maxval, min(length(CITY)) minval from STATION ) s2 where s1.namelength=s2.maxval or s1.namelength=s2.minval;